...
#pragma omp parallel for private(dx, dy, r, v1, v2, a, ax, ay, j) shared(local_data)
...
#pragma omp parallel for
        ...
        for (i = 0; i < NUM_BODIES; ++i){
            ...
            // refresh every 10 iterations
            if (count % 10 == 0) {
                omp_set_lock(&draw_lock);
                
                XDrawPoint (display, win, gc, bodies[i].x, bodies[i].y);
                usleep(10);
                
                omp_unset_lock(&draw_lock);
            }
        }
        omp_destroy_lock(&draw_lock);
        XFlush (display);
        usleep(100);
