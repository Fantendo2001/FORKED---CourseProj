...
int main(int argc, char* argv[])
{
    ...
    if (taskid == 0){
        int i, j;
#pragma omp parallel for private(j) shared(ROOM, X_RESN, Y_RESN)
        for (i = 0; i < X_RESN; ++i){
            for (j = 0; j < Y_RESN; ++j){
...
#pragma omp parallel for private(j) shared(start_row, end_row, local_data, ROOM, SIZE)
